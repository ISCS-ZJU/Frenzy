...
import math
from Frenzy import MARP, Scheduler as Frenzy_scheduler, Duration
    ...
    def try_allocate_job_to_cluster(self, job_a, cluster):
        ...
        for node in self.cluster.node_list:
            node_state = dict(
                ...
                tot_gpu_num=node.num_gpus/100, # num_gpus*100 in node.py
                free_gpu_num=math.floor(node.idl_gpus/100),
            )
            clus.append(node_state)
        
        # get a specific resources configuration
        res=Frenzy_scheduler.allocate_job_to_cluster(rsc_cfgs, clus)
        if res==-1:
            raise NotImplementedError("Resources in the entire cluster cannot meet this job.")
        elif res==0:
        ...
        trainCfg['MP'] = rsc_cfg['mp']
        trainCfg['train_steps'] = 1000
        trainCfg['FLOPS'] = 65*(10**12)    # select the minimum FLOPS among all allocated nodes

        duration = Duration.get_train_duration(modelCfg, trainCfg)
        job_a['duration']=math.floor(duration)
        job_a['num_inst']=trainCfg['DP']*trainCfg['MP']
        job_a['gpu_size']=rsc_cfg['gpu_size']

        ########### code added for Frenzy -  END  ###########
        ...
        if ig <= 0 and ic <= 0:
            ...
            for nid, node in enumerate(sorted_node_list):
                ...
                if self.gpu_size_matching == 1:     # GPU size perfect match. 
                    if job_a['gpu_size'] > node.gpu_size:
                        continue  # cannot on this node
                ##### code added for Frenzy -  END  #####
