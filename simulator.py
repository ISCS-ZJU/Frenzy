...
from cluster_Frenzy import Cluster
from node import Node
from scheduler_Frenzy import Scheduler
...
class Simulator:
    def __init__(self, csv_file, alloc_policy=0, preempt_policy=0,
                 ...
                 num_jobs_limit=None, gpu_type_matching=0, gpu_size_matching=0, verbose=0):
        self.cluster = None
        self.scheduler = None
        self.cur_time = None
        self.num_jobs = None
    ...
    def init_node_list_hetero(self):
        node_list = []
        node_id = 0
        for _ in range(20):  # low-ended machines first
            node_list.append(Node(node_id, 0, 96, gpu_type='CPU', gpu_size=0))
            node_id += 1
        for _ in range(8):  # low-ended machines first
            node_list.append(Node(node_id, 8, 96, gpu_type='A100', gpu_size=40))
            node_id += 1
        for _ in range(16):  # low-ended machines first
            node_list.append(Node(node_id, 2, 96, gpu_type='T4', gpu_size=16))
            node_id += 1
        for _ in range(20):  # low-ended machines first
            node_list.append(Node(node_id, 4, 64, gpu_type='3090', gpu_size=24))
            node_id += 1
        for _ in range(4):
            node_list.append(Node(node_id, 8, 96, gpu_type='A100', gpu_size=80))
            node_id += 1
        for _ in range(10):
            node_list.append(Node(node_id, 8, 96, gpu_type='V100', gpu_size=32))
            node_id += 1
        return node_list
    ...
    def init_go(self, num_jobs=None):
        ...
        self.scheduler = Scheduler(cluster=self.cluster, alloc_policy=self.alloc_policy,
                                   ...
                                   verbose=self.verbose, gpu_type_matching=self.gpu_type_matching, gpu_size_matching=self.gpu_size_matching)
