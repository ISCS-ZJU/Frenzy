...
CSV_FILE = 'pai_job_transformer_test.csv'
...
parser.add_argument("-g", "--num_gpus", help="Num of GPUs", type=int, default=20)
...
GPU_SIZE_MATCHING = 1 # GPU size match
# GPU_SIZE_MATCHING = 0  # ignore gpu size
...
comments = '%dg_%dn_h%d_%dp_%dsn_%dgt-%dgs-%dar-%dj-%dx-%dr' % (NUM_GPUS, NUM_NODES, HETERO, PATTERN, SORT_NODE_POLICY, GPU_TYPE_MATCHING, GPU_SIZE_MATCHING, ARRIVAL_RATE, NUM_JOBS, REPEAT, RANDOM_SEED)
...
for alloc_policy in [8]:  # 8FIFO (see utils.py)
    for preempt_policy in [2]:  # 2LGF
        ...
        simulator = Simulator(
            ...
            gpu_size_matching=GPU_SIZE_MATCHING,
            verbose=VERBOSE)
